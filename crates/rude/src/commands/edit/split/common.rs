...
pub(super) fn sort_by_kind(ranges: &mut [(usize, usize, &str, &str)]) {
...
pub(super) fn build_file_content(inner_attrs: &[String], use_lines: &[String], ranges: &[(usize, usize, &str, &str)], source_lines: &[&str]) -> String {
...
pub(super) fn find_mod_file(dir: &Path) -> Option<PathBuf> {
    [dir.join("lib.rs"), dir.join("mod.rs"), dir.join("main.rs")]
        .into_iter().find(|p| p.exists())
}

pub(super) fn insert_line_after(path: &Path, matcher: impl Fn(&str) -> bool, line: &str, skip_if: Option<&str>) -> Result<()> {
...
pub(super) fn insert_reexport(path: &Path, reexport_line: &str) -> Result<()> {
    insert_line_after(path, |t| t.starts_with("use ") || t.starts_with("pub use "), reexport_line, None)
}

pub(super) fn insert_mod_decl(path: &Path, mod_decl: &str, module_name: &str) -> Result<()> {
